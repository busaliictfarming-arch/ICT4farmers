...
app.post('/submit-form', (req, res) => {
    ...
    emailjs.send(emailParams.service_id, emailParams.template_id, emailParams.template_params)
        .then((response) => {
            ...
            res.redirect(`https://wa.me/+254113042231?text=Application%20Received%20from%20${encodeURIComponent(name)}%0AEmail:%20${encodeURIComponent(email)}%0AAnimal:%20${encodeURIComponent(animalType)}%0AQuantity:%20${encodeURIComponent(quantity)}%0AImage:%20${encodeURIComponent(animalImg ? animalImg : 'No image uploaded')}`);
        }, (error) => {
